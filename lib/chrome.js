...
export class StorageArea {
  ...
  set(items, callback) {
    ...
    function createOrUpdateItem(record) {
      function createItem() {
        storage.onChanged._addChange(areaName, record.id, null, record);
        return this_items.create(record, {useRecordId: true});
      }

      function updateItem(old_record) {
        storage.onChanged._addChange(areaName, record.id, old_record, record);
  ...
  remove(keys, callback) {
    ...
    function removeItem(key) {
      return this_items.delete(key).catch(err => {
