...
export class StorageArea {
  ...
  set(items, callback) {
    ...
    function createOrUpdateItem(record) {
      function createItem() {
        storage.onChanged._addChange(areaName, record.key, undefined, record.data);
        return this_items.create(record, {useRecordId: true});
      }

      function updateItem(old_record) {
        storage.onChanged._addChange(areaName, record.key, old_record, record.data);
  ...
  remove(keys, callback) {
    ...
    function removeItem(key) {
      return this_items.get(keyToId(key)).then(record => {
        if (!record) {
          return;
        }
        storage.onChanged._addChange(areaName, key, record.data.data, undefined);
        return this_items.delete(keyToId(key));
      }).catch(err => {
    ...
    this.items.list()
      .then(records => {
        const promises = records.data.map(record => {
          storage.onChanged._addChange(this.areaName, record.key, record.data, undefined);
          return this.items.delete(record.id);
        });
